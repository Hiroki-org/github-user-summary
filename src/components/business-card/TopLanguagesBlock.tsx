  ...
  <div className="space-y-8">
    ...
    {showTopics && topTopics.length > 0 && (
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {topTopics.map((topic: { name: string; count: number }) => (
            <span
              key={topic.name}
    ...
    {showInterests && interests && interests.topTopics.length > 0 && (
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {interests.topTopics.slice(0, 8).map((topic: { name: string; count: number }) => (
            <span
              key={topic.name}
    ...
    {showActivityBreakdown && activity && activity.eventBreakdown.length > 0 && (
      <div>
        ...
        <div className="space-y-2">
          {activity.eventBreakdown.slice(0, 5).map((event: { type: string; count: number }) => (
