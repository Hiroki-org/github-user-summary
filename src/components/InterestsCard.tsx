...
export default function InterestsCard({ interests }: Props) {
  ...
  if (totalStarred === 0 || (topTopics.length === 0 && topLanguages.length === 0)) {
    return null;
  }
  ...
  return (
    <div className="rounded-lg border border-card-border bg-card-bg p-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-sm text-muted">⭐ {totalStarred.toLocaleString()} starred</span>
      </div>
      ...
      {topLanguages.length > 0 && (
        <div>
          <h4 className="mb-2 text-sm font-medium text-muted">Interest Languages</h4>
          <div className="space-y-2">
            {topLanguages.slice(0, 8).map((lang) => (
              <div key={lang.name} className="flex items-center justify-between rounded-md border border-card-border px-3 py-2 text-sm">
