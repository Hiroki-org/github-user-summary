...
const generateValidSigUrl = (username: string) => {
  const secret = "default_og_secret_for_local_dev";
  const sig = crypto.createHmac("sha256", secret).update(username).digest("hex");
  return `http://localhost/api/og/${encodeURIComponent(username)}?sig=${sig}`;
};

describe("OG Image Route", () => {
  it("should return 401 if sig is missing", async () => {
  ...
  it("should return 401 if sig is invalid", async () => {
    const req = new NextRequest("http://localhost/api/og/validuser?sig=invalid");
    const res = await GET(req, { params: Promise.resolve({ username: "validuser" }) });
    expect(res.status).toBe(401);
  });
  ...
  it("should generate image for valid username with authorization header when GITHUB_TOKEN is set", async () => {
    ...
    const req = new NextRequest(generateValidSigUrl("validuser"));
