...
import { loadCardSettings, type CardDisplayOptions } from "@/lib/cardSettings";
...
export function useCardSettings(mounted: boolean) {
  ...
  const [displayOptions, setDisplayOptions] = useState<CardDisplayOptions>(() => {
    try {
      return loadCardSettings().options;
    } catch {
      // Fallback for tests or environments where localStorage throws
      return {} as CardDisplayOptions;
    }
  });
