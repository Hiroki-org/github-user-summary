...
export function useYearInReview(year: number | null) {
    const hasValidYear = Number.isFinite(year);
    const query = year === null ? null : `/api/dashboard/year?year=${year}`;
    const { data, isLoading, error, mutate } = useAuthenticatedFetch<YearInReviewData>(query, { enabled: hasValidYear });
...
export function useDashboardStats(year: number | null) {
    const hasValidYear = Number.isFinite(year);
    const query = year === null ? null : `/api/dashboard/stats?year=${year}`;
    const { data, isLoading, error, mutate } = useAuthenticatedFetch<DashboardStatsResponse>(query, { enabled: hasValidYear });
