...
describe("githubYearInReview granular coverage", () => {
    it("fetchCommitDatesForTopRepos handles null target or null nodes", async () => {
        mockFetch.mockImplementation(() => {
            return Promise.resolve(jsonResponse({
                data: {
                    user: {
                    ...
                    repo0: { defaultBranchRef: { target: null } }
                }
            }));
        });
        const data = await fetchYearInReviewData("user", 2024, "token");
        ...
        mockFetch.mockImplementation(() => {
            return Promise.resolve(jsonResponse({
                data: {
                    ...
                    repo0: { defaultBranchRef: { target: { history: { nodes: null } } } }
                }
            }));
        });
        const data2 = await fetchYearInReviewData("user", 2024, "token");
        expect(data2.year).toBe(2024);
    });

    it("fetchCommitDatesForTopRepos handles invalid history structure", async () => {
        mockFetch.mockImplementation(() => {
            return Promise.resolve(jsonResponse({
                data: {
                    user: {
                    ...
                    repo0: { defaultBranchRef: { target: { history: "invalid" } } }
                }
            }));
        });
        const data = await fetchYearInReviewData("user", 2024, "token");
