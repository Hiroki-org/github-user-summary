...
describe('apiUtils', () => {
  ...
  describe('getAuthAndYear', () => {
    it('returns 401 response if no user', async () => {
      ...
      expect((result.errorResponse as unknown as { init?: { status?: number } })?.init?.status).toBe(401);
    });
    ...
    it('returns 400 if year param has invalid characters', async () => {
      ...
      const result = await getAuthAndYear(req);
      expect(result.errorResponse).toBeDefined();
      expect((result.errorResponse as unknown as { init?: { status?: number } })?.init?.status).toBe(400);
    });
