...
function aggregateTopics(repos: StarredRepo[]): Map<string, number> {
...
function aggregateLanguages(repos: StarredRepo[]): Map<string, number> {
...
export const fetchActivity = cache(async function fetchActivity(
  ...
  promises.forEach((p) =>
    p.catch((e) => logger.error("Event fetch promise rejected:", e)),
  );
