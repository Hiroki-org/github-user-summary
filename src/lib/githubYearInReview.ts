...
function mergeTopRepository(data: NonNullable<YearInReviewResponse["user"]>["contributionsCollection"]): { name: string; contributions: number } | null {
    ...
    const processBuckets = (buckets?: ContributionsByRepoNode[]) => {
        if (!buckets) return;
    ...
    processBuckets(data.commitContributionsByRepository);
    processBuckets(data.pullRequestContributionsByRepository);
    processBuckets(data.issueContributionsByRepository);
